...
type Context struct {
	//从pod创建的application都会被添加到这里？
	//informer发现的pod，所属的app被添加到这里
...
func (ctx *Context) AddSchedulingEventHandlers() {
	...
	ctx.podInformer.Informer().AddEventHandler(
		cache.FilteringResourceEventHandler{
			FilterFunc: ctx.filterPods,   //过滤掉不属于该调度器调度的pod
    ...
    //todo：有了上面那个，为什么还需要这个？
	ctx.podInformer.Informer().AddEventHandler(
		cache.FilteringResourceEventHandler{
			FilterFunc: ctx.filterAssignedPods,     //过滤掉已经指定node的pod，即已调度的pod，即pod.Spec.NodeName != 0
			Handler: cache.ResourceEventHandlerFuncs{
...
func (ctx *Context) addPod(obj interface{}) {
	...
	if pod.Status.Phase == v1.PodPending {
		...
		//根据pod的appid获取application，如果之前已经有pod具有相同的appid，那么application已经存在与context中了，那么直接从context中获取那个app
		//如果是第一个，那么创建新的app
		if app := ctx.getOrCreateApplication(pod); app != nil {
			//获取task并添加到对应的app中，但还没调度
...
//ApplicationEvent及其子事件触发，由dispatcher负责事件调度
func (ctx *Context) ApplicationEventHandler() func(obj interface{}){
	return func(obj interface{}) {
		if event, ok := obj.(events.ApplicationEvent); ok {
			...
			//具体是ApplicationEvent的哪个子事件，以及对应的不同处理逻辑，由Application这个类具体去实现
...
//TaskEvent及其子事件触发，由dispatcher负责事件调度
func (ctx *Context) TaskEventHandler() func(obj interface{}){
	return func(obj interface{}) {
		if event, ok := obj.(events.TaskEvent); ok {
			...
			//具体是TaskEvent的哪个子事件，以及对应的不同处理逻辑，由Task这个类具体去实现
