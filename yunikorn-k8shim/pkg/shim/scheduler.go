...
type KubernetesShim struct {
	rmProxy        api.SchedulerApi       //core接口
	context        *cache.Context         //
	callback       api.ResourceManagerCallback   //core回调
...
func newShimSchedulerInternal(api api.SchedulerApi, ctx *cache.Context, cb api.ResourceManagerCallback) *KubernetesShim {
	...
	ss.stateMachine = fsm.NewFSM(
		...
		fsm.Callbacks{
			...
			//后面三个事件是连着发生的，由RegisterSchedulerSucceed事件触发
...
//shim的状态为running后，调用该函数开始调度；
//该函数首先为informer注册事件，然后开启线程进行调度主循环
func (ss *KubernetesShim) doScheduling() func(e *fsm.Event) {
	return func(e *fsm.Event) {
		...
		//每隔conf.GetSchedulerConf().GetSchedulingInterval()时间，触发一次schedule()函数
...
//遍历所有app，如果有task处于pending状态，那么根据这个app的当前状态完成状态的改变，并触发相应的行为
//informer获取pod后生成对应的task（如果没有对应的app，同时会生产app），但是只是生成了，并没有调度，这里就是去调度这些app和task
func (ss *KubernetesShim) schedule() {
	...
	for _, app := range apps {
		for _, pendingTask := range app.GetPendingTasks() {
			...
			case states.Accepted:   //app已经被accepted了，但是task还处于pending状态
